defmodule Noizu.ConfigHelper.Extension do
        ...
        case as do
          ...
          type when type in [:bool, :flag] ->
            cond do
              ...
              required in [true, :required] ->
                raise("#{env.file}:#{env.line}  Config Error - User must set #{key}=[...] environment variable as bool, got: [#{v}]")
              :else ->
                IO.puts("#{env.file}:#{env.line} Config Error - User should set #{key}=[...] environment variable as bool, got: [#{v}]")
                default
            end
  ...
  defmacro env_setting(key, options \\ [default: false, required: true, as: :string, prefix: true]) do
    ...
    quote do
      Noizu.ConfigHelper.Extension.env_as(unquote(key), unquote(as), __ENV__, unquote(default), unquote(required), unquote(prefix))
    end
  end
